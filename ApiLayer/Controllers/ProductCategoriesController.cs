...
namespace ApiLayer.Controllers
{
    ...
    public class ProductCategoriesController : ControllerBase
    {
        ...
        public async Task<ActionResult<int>> GetCount()
        {
            try
            {
                var count = await _productCategoryService.GetCountOfAsync();
                return Ok(count);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("range", Name = "AddRangeProductCategories")]
        [Authorize(Roles = Role.Admin)]
        [ProducesResponseType(200)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        [ProducesResponseType(401)]
        public async Task<ActionResult<string>> AddRangeProductCategories(IEnumerable< ProductCategoryDto> productCategoriesDtos)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState);
            ...
            try
            {
                ...
                var NewproductCategoryDto = await _productCategoryService.AddRangeAsync(productCategoriesDtos, UserId);

                if (NewproductCategoryDto == null || !NewproductCategoryDto.Any()) return BadRequest("Cannot add new product Categories");
